...
Canvas provides spawnable terminal displays (calendars, documents, flight booking, virtual teaching assistant) with real-time IPC communication. Claude can spawn these TUIs in tmux split panes and receive user selections.
...
| `vta` | Virtual Teaching Assistant for hands-on labs |

## Lab Environment

The vTA (Virtual Teaching Assistant) provides hands-on learning labs with:
- Real-time task validation (command-based and result-based)
- Docker container for safe practice environment
- Auto-detection of completed tasks

```bash
# Start the Linux User Management lab
bun run src/cli.ts lab linux-user-management
```

See [docs/LAB_ENVIRONMENT.md](docs/LAB_ENVIRONMENT.md) for full documentation.
